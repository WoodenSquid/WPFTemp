...
using WPFUITemplate2.Models.Monaco;

namespace WPFUITemplate2.Controllers
{
    public class MonacoController
    {
        ...
        public async Task SetThemeAsync(ThemeType appTheme)
        {
            ...
            await _webView.ExecuteScriptAsync(
                " monaco.editor.defineTheme('{{" + uiThemeName + "}}', { base: '{{" + baseMonacoTheme + "}}', inherit: true, rules: [{ background: 'FFFFFF00' }], colors: {'editor.background': '#FFFFFF00','minimap.background': '#FFFFFF00',}}); monaco.editor.setTheme('{{" + uiThemeName + "}}'); ");
        }
